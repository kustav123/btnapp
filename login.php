...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err)){
        ...
        $sql = "SELECT id, fname, lname, uname, role, photo, password FROM user WHERE uname = ? and status = 1";
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $fname, $lname, $uname, $role, $photo, $hashed_password);
